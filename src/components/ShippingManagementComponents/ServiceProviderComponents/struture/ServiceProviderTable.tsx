...
const ServiceProviderTable = () => {
    ...
    const cityItem: {key: string, label: string}[]= [
    ...
    const selectedCityLabel = cityItem.find((item) => item.key === selectedCity)?.label || t("City");
    const selectedStatusLabel = statusItems(t).find((item) => item.key === selectedStatus)?.label || t("Status");
            ...
            <Card className='radius-12 border-gray card-cs h-100'>
                ...
                <Flex vertical gap={20}>
                    <Table<ServiceProviderType>
                        size='large'
                        columns={serviceproviderColumn({ setVisible,setEditItem, navigate, setStatusChanged, setDeleteItem },t)}
